...
std::string _ledGreenEnable_ =
      "background-color: rgb(0, 192, 0);"
      "border: 1px inset #201F1F;";

std::string _ledGreenDisable_ =
      "background-color: rgb(0, 48, 0);"
      "border: 1px inset #201F1F;";

std::string _ledYellowEnable_ =
      "background-color: rgb(255, 255, 0);"
      "border: 1px inset #201F1F;";

std::string _ledYellowDisable_ =
      "background-color: rgb(64, 64, 0);"
      "border: 1px inset #201F1F;";

std::string _ledBlueEnable_ =
      "background-color: rgb(51, 51, 255);"
      "border: 1px inset #201F1F;";

std::string _ledBlueDisable_ =
      "background-color: rgb(13, 13, 64);"
      "border: 1px inset #201F1F;";

std::string _ledBrownEnable_ =
      "background-color: rgb(128, 0, 0);"
      "border: 1px inset #201F1F;";

std::string _ledBrownDisable_ =
      "background-color: rgb(32, 0, 0);"
      "border: 1px inset #201F1F;";

std::string _ledRedEnable_ =
      "background-color: rgb(255, 0, 0);"
      "border: 1px inset #201F1F;";

std::string _ledRedDisable_ =
...
SSMix::SSMix () : ossmix ( SilentMedia::SoundSystem::OSS::Mixer::Instance() ), /*sscc ( new SScc ),*/
      peak ( new Peak ), ctrlNum ( -1 ), numRecDev ( -1 ), ctrlParent ( -1 ), recModeAvail ( 0 ),
recModeStatus ( 0 ), ctrlMode ( 0 ), ctlStatus ( -1 ), L ( -1 ), R ( -1 ), M ( -1 ), minCtrlValue ( -1 ),
maxCtrlValue ( -1 ), skipDev ( 0 ), ctrlFlag ( -1 ), currentEnumNum ( -1 ), updateCounter ( -1 ),
currentParent ( -1 )
{ }
...
// 	tmpL = new QLabel/* ( "asdas" )*/;
   ...
   pConfigFile = new std::fstream;
   ptmpConfigFile = new std::fstream;

   ptmpConfigFile -> open ( currentConfigFile.c_str(), std::fstream::trunc | std::fstream::in | std::fstream::out );
   *ptmpConfigFile << "<mixer>" << std::endl;
...
//          delete img [ id ]; img [ id ] = 0;
   ...
   for ( std::map < int, std::string > :: iterator it = listOfAvaibleCtrlDev.begin ( ); it != listOfAvaibleCtrlDev.end ( ); ++it ) {
      ...
      ossmix -> getDevInfo ( it -> second, ctrlLabel, ctrlNum, ctrlParent, numRecDev, recModeAvail, recModeStatus, ctrlMode,
                              ctlStatus, L, R, M, minCtrlValue, maxCtrlValue, skipDev, enumListVariant, currentEnumName,
                              currentEnumNum, ctrlTypeName, ctrlFlag );

      if ( !skipDev ) {
         *ptmpConfigFile
               << "\t" << "<control id='" << ctrlNum << "'>" << std::endl
               << "\t\t" << "<name>" << ctrlLabel << "</name>" << std::endl
               << "\t\t" << "<type>" << ctrlTypeName << "</type>" << std::endl
               << "\t\t" << "<chanels>" << "merge" << "</chanels>" << std::endl
               << "\t\t" << "<record>" << recModeStatus << "</record>" << std::endl
               << "\t\t" << "<mute>" << std::endl
               << "\t\t\t" << "<L>" << "0" << "</L>" << std::endl
               << "\t\t\t" << "<R>" << "0" << "</R>" << std::endl
               << "\t\t" << "</mute>" << std::endl
         ...
         std::cout << id << " : " << ctrlNum << std::endl;
         vIdtoCtrlRecNum [ id ] = numRecDev;
         mctrlMode [ id ] = ctrlMode;
         ctrlMinVal [ id ] = minCtrlValue;
         ctrlMaxVal [ id ] = maxCtrlValue;

//   std::cout << "MIXT: " << ctrlTypeName << std::endl;
//   std::cout << "label: " << ctrlLabel << std::endl;
//   std::cout << "ctrlMode: " << ctrlMode << std::endl << std::endl;
         ...
         if ( ( ctrlTypeName == MIXT_STEREOSLIDER ) || ( ctrlTypeName == MIXT_STEREODB ) ||
               ( ctrlTypeName == MIXT_MONOSLIDER ) || ( ctrlTypeName == MIXT_MONODB ) ||
               ( ctrlTypeName == MIXT_MONOVU ) || ( ctrlTypeName == MIXT_SLIDER ) ||
               ( ctrlTypeName == MIXT_STEREOSLIDER16 ) || ( ctrlTypeName == MIXT_MONOSLIDER16 ) ) {
            ...
            } else if ( ( ctrlTypeName == MIXT_STEREOSLIDER16 ) || ( ctrlTypeName == MIXT_MONOSLIDER16 ) ) {
...
//                   IdtoPicName [ id ] = loadImg;
//                   std::cout << sscc -> P ( Color ( red ) ) << "[ QSSMix ] [ ERROR: ] [ Value of `" << ctrlLabel << "` control = " << L << " : " << R << " ] " << sscc -> P ( Color ( white ) ) << std::endl;
// #ifdef QSSMIX_DEBUG
//                   std::cout << std::endl << sscc -> P ( Color ( cyan ) ) << "[ QSSMix ] [ DEBUG MODE START: ]" << std::endl;
//                   std::cout << "\t" << "Control name: " << ctrlLabel << std::endl;
//                   std::cout << "\t" << "Control type: " << ctrlTypeName << std::endl;
//                   std::cout << "\t" << "Control number: " << ctrlNum << std::endl;
//                   std::cout << "\t" << "Record mode availability: " << recModeAvail << std::endl;
//                   std::cout << "\t" << "Record mode status: " << recModeStatus << std::endl;
//                   std::cout << "\t" << "MONO / STEREO: " << ctrlMode << std::endl;
//                   std::cout << "[ QSSMix ] [ DEBUG MODE END. ]" << sscc -> P ( Color ( white ) ) << std::endl << std::endl;
// #endif
//                }
            } else if ( !ctrlMode ) {
               if ( M == minCtrlValue ) {
               ...
               loadImg = "account_offline_overlay.png";
               IdtoPicName [ id ] = loadImg;
               std::cout << sscc -> P ( Color ( red ) ) << "[ QSSMix ] [ ERROR: ] [ Value of `" << ctrlLabel << "` control = " << M << " ]" << sscc -> P ( Color ( white ) ) << std::endl;
#ifdef QSSMIX_DEBUG
               std::cout << std::endl << sscc -> P ( Color ( cyan ) ) << "[ QSSMix ] [ DEBUG MODE START: ]" << std::endl;
               std::cout << "\t" << "Control name: " << ctrlLabel << std::endl;
               std::cout << "\t" << "Control type: " << ctrlTypeName << std::endl;
            ...
            if ( ctrlMode ) {
               muteLRAct [ id ] = new QAction ( QIcon ( ":/img/mute.png" ), tr ( "&Mute" ), this );
               reverseAct [ id ] = new QAction ( QIcon ( ":/img/xfce4_xicon.png" ), tr ( "&Reverse chanels" ), this );
               splitChanelAct [ id ] = new QAction ( QIcon ( ":/img/xfce4_xicon.png" ), tr ( "&Split chanels" ), this );
            ...
            if ( recModeAvail ) {
               setRecordSrcAct [ id ] = new QAction ( QIcon ( ":/img/rec.png" ), tr ( "&Set record source" ), this );
// 						setRecordSrcAct [ id ] -> setCheckable ( true );
            ...
            if ( ctrlMode ) {
               ...
               connect ( reverseAct [ id ] , SIGNAL ( triggered ( ) ), reverseActSignalMapper, SLOT ( map ( ) ) );
               reverseActSignalMapper -> setMapping ( reverseAct [ id ], id );

               connect ( splitChanelAct [ id ] , SIGNAL ( triggered ( ) ), splitChanelActSignalMapper, SLOT ( map ( ) ) );
            ...
            if ( recModeAvail ) {
               connect ( setRecordSrcAct [ id ] , SIGNAL ( triggered ( ) ), setRecordSrcSignalMapper, SLOT ( map ( ) ) );
            ...
            if ( ( ctrlTypeName == MIXT_STEREOSLIDER16 ) || ( ctrlTypeName == MIXT_MONOSLIDER16 ) ) {
// 						std::cout << "16 " << id << std::endl;
               ...
               else if ( ( ctrlTypeName == MIXT_STEREOVU ) || ( ctrlTypeName == MIXT_STEREOPEAK ) ) {
                  if ( !skipDev ) {
...
void SSMix::setIconStatus ( int id, bool status ) {
   if ( ctrlStatus [ id ] != status ) {
      ctrlStatus [ id ] = status;
               status ? icon [ id ] -> addPixmap ( ( QIcon ( ( IdtoPicName [ id ] ) . c_str ( ) ) ) . pixmap ( QSize ( 32, 32 ), QIcon::Normal, QIcon::On ) ) :
                     icon [ id ] -> addPixmap ( ( QIcon ( ( IdtoPicName [ id ] ) . c_str ( ) ) ) . pixmap ( QSize ( 32, 32 ), QIcon::Disabled, QIcon::On ) );
               ImgButton [ id ] -> setIcon ( *icon [ id ] );
   }
}
...
void SSMix::setVol ( const int id, bool mode, int &setVolL, int &setVolR, int &setVolM, int &devNum ) {
...
// 	l -> repaint();
   ...
   if ( ossmix -> getUpdateCounter ( ) > updateCounter ) {
      ...
      int id = 0;
      for ( std::map < int, std::string > :: iterator it = listOfAvaibleCtrlDev.begin ( ); it != listOfAvaibleCtrlDev.end ( ); ++it ) {
         ossmix -> getDevInfo ( it->second, ctrlLabel, ctrlNum, ctrlParent, numRecDev, recModeAvail,
                                 recModeStatus, ctrlMode, ctlStatus, L, R, M, minCtrlValue, maxCtrlValue, skipDev, enumListVariant, currentEnumName, currentEnumNum, ctrlTypeName, ctrlFlag );
...
void SSMix::saveConfig ( ) {
   std::ifstream infile ( currentConfigFile.c_str(), std::ios_base::binary );
   std::ofstream outfile ( configFile.c_str(), std::ios_base::binary );
